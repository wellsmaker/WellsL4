...
'''Common definitions for building WellL4 applications.

This provides some default settings and convenience wrappers for
building WellL4 applications needed by multiple commands.
...
'''Name of the default WellL4 build directory.'''
...
checked, in that order. If one is a WellL4 build directory, it is used.
...
def find_build_dir(dir, guess=False, **kwargs):
    ...
    if dir:
        ...
        if default and is_wellsl4_build(default):
            build_dir = default
        elif is_wellsl4_build(cwd):
            build_dir = cwd
        else:
            build_dir = default
...
def is_wellsl4_build(path):
    '''Return true if and only if `path` appears to be a valid WellL4
    build directory.

    "Valid" means the given path is a directory which contains a CMake
    cache with a 'WELLSL4_TOOLCHAIN_VARIANT' key.
    '''
    try:
    ...
    if 'WELLSL4_TOOLCHAIN_VARIANT' in cache:
        log.dbg('{} is a wellsl4 build directory'.format(path),
                level=log.VERBOSE_EXTREME)
        return True
    else:
        log.dbg('{} is NOT a valid wellsl4 build directory'.format(path),
                level=log.VERBOSE_EXTREME)
        return False
