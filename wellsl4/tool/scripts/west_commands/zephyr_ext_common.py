...
def cached_runner_config(build_dir, cache):
    ...
    board_dir = cache['WELLSL4_RUNNER_CONFIG_BOARD_DIR']
    elf_file = cache.get('WELLSL4_RUNNER_CONFIG_KERNEL_ELF')
    hex_file = cache.get('WELLSL4_RUNNER_CONFIG_KERNEL_HEX')
    bin_file = cache.get('WELLSL4_RUNNER_CONFIG_KERNEL_BIN')
    gdb = cache.get('WELLSL4_RUNNER_CONFIG_GDB')
    openocd = cache.get('WELLSL4_RUNNER_CONFIG_OPENOCD')
    openocd_search = cache.get('WELLSL4_RUNNER_CONFIG_OPENOCD_SEARCH')
