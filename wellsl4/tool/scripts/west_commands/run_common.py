...
from runners import get_runner_cls, WellsL4BinaryRunner, MissingProgram

from wellsl4_ext_common import cached_runner_config
...
def add_parser_common(parser_adder, command):
    ...
    group.add_argument('-c', '--cmake-cache',
                       ...
                       configuration (this is generated by the WellL4
    ...
    group = parser.add_argument_group(
        ...
        These values usually come from the WellL4 build system itself
...
def _build_dir(args, die_if_none=True):
    ...
    if dir and is_wellsl4_build(dir):
        return dir
    elif die_if_none:
        msg = '--build-dir was not given, '
        if dir:
            msg = msg + 'and neither {} nor {} are wellsl4 build directories.'
        else:
...
def do_run_common(command, args, runner_args, cached_runner_var):
    ...
    # line override. Get the WellsL4BinaryRunner class by name, and
    ...
    available = cache.get_list('WELLSL4_RUNNERS')
    if not available:
    ...
    cached_runner_args = cache.get_list(
        'WELLSL4_RUNNER_ARGS_{}'.format(cmake.make_c_identifier(runner)))
...
def _dump_context(command, args, runner_args, cached_runner_var):
    ...
    all_cls = {cls.name(): cls for cls in WellsL4BinaryRunner.get_runners() if
               command.name in cls.capabilities().commands}
    available = [r for r in cache.get_list('WELLSL4_RUNNERS') if r in all_cls]
    available_cls = {r: all_cls[r] for r in available if r in all_cls}

    default_runner = cache.get(cached_runner_var)
    cfg = cached_runner_config(build_dir, cache)

    log.inf('All WellL4 runners which support {}:'.format(command.name),
            colorize=True)
...
def _dump_one_runner_info(cache, args, build_dir, indent):
    ...
    available = runner in cache.get_list('WELLSL4_RUNNERS')
    cfg = cached_runner_config(build_dir, cache)
...
def _get_runner_args(cache, runner):
    ...
    args_var = 'WELLSL4_RUNNER_ARGS_{}'.format(runner_ident)
