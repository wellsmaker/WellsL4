...
def read_intlist(intlist_path, syms):
    ...
    Followed by instances of struct _isr_list created by IRQ_DYNC_CONNECT()
    calls:
...
def parse_args():
    ...
    parser.add_argument("-i", "--intlist", required=True,
            help="WellL4 intlist binary for intList extraction")
...
#define ISR_WRAPPER ((u32_t)&isr_wrapper)
#else
...
def write_source_file(fp, vt, swt, intlist, syms):
    ...
    if vt:
        fp.write("u32_t __irq_vector_table irq_vector_table[%d] = {\n" % nv)
        for i in range(nv):
            fp.write("\t{},\n".format(vt[i]))
        fp.write("};\n")
    ...
    fp.write("struct isr_table_entry __sw_isr_table sw_isr_table[%d] = {\n"
            % nv)
...
def main():
    ...
    for irq, flag, func, param in intlist["interrupts"]:
        if flag & isr_direct_isr:
            if param != 0:
            ...
            if swt[table_index] != (0, spurious_handler):
                error(f"multiple registrations at table_index {table_index} for irq {irq} (0x{irq:x})"
                      + "\nHas IRQ_DYNC_CONNECT or IRQ_DIRECT_CONNECT accidentally been invoked on the same irq multiple times?"
                )
