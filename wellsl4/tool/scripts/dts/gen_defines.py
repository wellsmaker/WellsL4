...
def main():
    ...
    # Derived from /chosen
    write_addr_size(edt, "wellsl4,sram", "SRAM")
    write_addr_size(edt, "wellsl4,ccm", "CCM")
    write_addr_size(edt, "wellsl4,dtcm", "DTCM")
    write_addr_size(edt, "wellsl4,ipc_shm", "IPC_SHM")
...
def relativize(path):
    # If 'path' is within $WELLSL4_BASE, returns it relative to $WELLSL4_BASE,
    # with a "$WELLSL4_BASE/..." hint at the start of the string. Otherwise,
    # returns 'path' unchanged.

    zbase = os.getenv("WELLSL4_BASE")
    if zbase is None:
        return path

    try:
        return str("$WELLSL4_BASE" / pathlib.Path(path).relative_to(zbase))
    except ValueError:
        # Not within WELLSL4_BASE
        return path
...
def write_flash_node(edt):
    ...
    # wellsl4,flash in /chosen

    node = edt.chosen_node("wellsl4,flash")

    out_comment("/chosen/wellsl4,flash ({})"
    ...
    if len(node.regs) != 1:
        err("expected wellsl4,flash to have a single register, has {}"
...
def write_code_partition(edt):
    # Writes output for the node pointed at by wellsl4,code-partition in /chosen

    node = edt.chosen_node("wellsl4,code-partition")

    out_comment("/chosen/wellsl4,code-partition ({})"
                .format(node.path if node else "missing"))
...
def write_irqs(node):
    ...
    def encode_wellsl4_multi_level_irq(irq, irq_num):
    ...
    for irq_i, irq in enumerate(node.interrupts):
        for cell_name, cell_value in irq.data.items():
            ...
            if cell_name == "irq":
                ...
                cell_value = encode_wellsl4_multi_level_irq(irq, cell_value)
            else:
...
def write_clocks(node):
    ...
    #
    # See https://github.com/wellsl4project-rtos/wellsl4/pull/19327#issuecomment-534081845.
