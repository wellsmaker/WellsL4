...
function handle_coverage() {
	...
	echo "Calling gcovr"
	gcovr -r ${WELLSL4_BASE} -x > shippable/codecoverage/coverage.xml
...
function handle_compiler_cache() {
	...
	if [ -f "$HOME/.cache/wellsl4/ToolchainCapabilityDatabase.cmake" ]; then
		echo "Dumping the capability database in case we are affected by #9992"
		cat $HOME/.cache/wellsl4/ToolchainCapabilityDatabase.cmake
	fi
}

function on_complete() {
	source wellsl4-env.sh
	if [ "$1" == "failure" ]; then
		handle_compiler_cache
	fi

	rm -rf ccache $HOME/.cache/wellsl4
...
function run_bsim_bt_tests() {
	WORK_DIR=${WELLSL4_BASE}/bsim_bt_out tests/bluetooth/bsim_bt/compile.sh
	RESULTS_FILE=${WELLSL4_BASE}/${bsim_bt_test_results_file} \
...
function get_tests_to_run() {
	./scripts/wellsl4_module.py --sanitycheck-out module_tests.args
...
if [ -n "$main_ci" ]; then
	...
	source wellsl4-env.sh
	sanitycheck="${WELLSL4_BASE}/scripts/sanitycheck"
