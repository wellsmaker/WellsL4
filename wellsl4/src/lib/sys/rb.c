...
{
	assert(get_child(parent, 0) == child || get_child(parent, 1) == child);
...
static void fix_extra_red(struct rbnode **stack, int stacksz)
{
	while (stacksz > 1) 
	{
		...
		assert(!get_child(node, 0) || is_black(get_child(node, 0)));
		assert(!get_child(node, 1) || is_black(get_child(node, 1)));
...
{
	node->children[0] = (void *)0;
	node->children[1] = (void *)0;
}
	...
	struct rbnode *null_node)
{
	...
	while (stacksz > 1) 
	{
		...
		assert((c0 && is_red(c0)) || (c1 && is_red(c1)));
...
void rb_remove(struct rbtree *tree, struct rbnode *node)
{
	...
	assert(!get_child(node, 0) || !get_child(node, 1));
...
void rb_walk_of(struct rbnode *node, rb_visit_t visit_fn, void *cookie)
{
	if (node != NULL) 
	{
		rb_walk_of(get_child(node, 0), visit_fn, cookie);
		visit_fn(node, cookie);
		rb_walk_of(get_child(node, 1), visit_fn, cookie);
	}
}
#endif
 ...
 */
static FORCE_INLINE struct rbnode *stack_left_limb(struct rbnode *n,
 ...
 * alloca().  The _current_thread node is found in stack[top] (and its parent
