...
static sword_t next(struct lexer *lexer)
{
...
static sword_t peek(struct lexer *lexer)
{
	sword_t chr = next(lexer);
...
static sword_t accept_run(struct lexer *lexer, const char *run)
{
	for (; *run; run++) 
	{
		if (next(lexer) != *run)
...
static sword_t obj_init(struct json_obj *json, char *data, size_t len)
{
	struct token token;
...
static sword_t element_token(enum json_tokens token)
{
	switch (token) 
	{
...
static sword_t obj_next(struct json_obj *json,
...
static sword_t arr_next(struct json_obj *json, struct token *value)
{
...
static sword_t decode_num(const struct token *token, sword_t *num)
{
...
static sword_t obj_parse(struct json_obj *obj,
		     const struct json_obj_descr *descr, size_t descr_len,
		     void *val);
static sword_t arr_parse(struct json_obj *obj,
		     const struct json_obj_descr *elem_descr,
		     size_t max_elements, void *field, void *val);

static sword_t decode_value(struct json_obj *obj,
...
static sword_t obj_parse(struct json_obj *obj, const struct json_obj_descr *descr,
		     size_t descr_len, void *val)
{
	struct json_obj_key_value kv;
	sword_t decoded_fields = 0;
	size_t i;
	sword_t ret;
...
sword_t json_obj_parse(char *payload, size_t len,
		   const struct json_obj_descr *descr, size_t descr_len,
		   void *val)
{
	struct json_obj obj;
...
static sword_t json_escape_internal(const char *str,
...
static sword_t encode(const struct json_obj_descr *descr, const void *val,
		  json_append_bytes_t append_bytes, void *data);

static sword_t arr_encode(const struct json_obj_descr *elem_descr,
...
static sword_t str_encode(const char **str, json_append_bytes_t append_bytes,
		      void *data)
{
	sword_t ret;
...
static sword_t num_encode(const sword_t *num, json_append_bytes_t append_bytes,
		      void *data)
{
	char buf[3 * sizeof(sword_t)];
	sword_t ret;
...
static sword_t bool_encode(const bool *value, json_append_bytes_t append_bytes,
		       void *data)
{
	if (*value) 
	{
...
sword_t json_obj_encode(const struct json_obj_descr *descr, size_t descr_len,
...
static sword_t append_bytes_to_buf(const char *bytes, size_t len, void *data)
{
...
sword_t json_obj_encode_buf(const struct json_obj_descr *descr, size_t descr_len,
...
static sword_t measure_bytes(const char *bytes, size_t len, void *data)
{
	ssize_t *total = data;
