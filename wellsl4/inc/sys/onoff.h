...
struct onoff_service {
	...
	/* Mutex protection for flag, clients, releaser, and refs. */
	spinlock_t lock;
 ...
 * Include @ref ONOFF_SERVICE_RESET_SLEEPS as appropriate in flag.
 *
 * @param flag any or all of the flag mentioned above,
 * e.g. @ref ONOFF_SERVICE_START_SLEEPS.  Use of other flag produces an
 * error.
 *
 * @retval 0 on success
 * @retval -EINVAL if start, stop, or flag are invalid
 */
 ...
 *
 * These flag control whether calls to onoff_service_request() and
 ...
 * pre-kernel, ISR, or cooperative or pre-emptible record_threads.
 ...
 */
static FORCE_INLINE sword_t onoff_client_fetch_result(const struct onoff_client *op,
					    sword_t *result)
{
	assert(op != NULL);
	assert(result != NULL);
 ...
 */
static FORCE_INLINE void onoff_client_init_spinwait(struct onoff_client *cli)
{
	assert(cli != NULL);

	*cli = (struct onoff_client) {
		.flag = ONOFF_CLIENT_NOTIFY_SPINWAIT,
	};
}
 ...
 */
static FORCE_INLINE void onoff_client_init_signal(struct onoff_client *cli,
...
#endif /* CONFIG_POLL */
		.flag = ONOFF_CLIENT_NOTIFY_SIGNAL,
	};
}
 ...
 * additional context.
 */
static FORCE_INLINE void onoff_client_init_callback(struct onoff_client *cli,
					      onoff_client_callback handler,
					      void *user_data)
{
	assert(cli != NULL);
	...
	*cli = (struct onoff_client){
		...
		.flag = ONOFF_CLIENT_NOTIFY_CALLBACK,
	};
}
 ...
 */
sword_t onoff_request(struct onoff_service *srv,
	struct onoff_client *cli);
 ...
 */
sword_t onoff_release(struct onoff_service *srv,
	struct onoff_client *cli);
 ...
 */
static FORCE_INLINE bool onoff_service_has_error(const struct onoff_service *srv)
{
	return (srv->flag & ONOFF_SERVICE_HAS_ERROR) != 0;
}
 ...
 */
sword_t onoff_service_reset(struct onoff_service *srv,
	struct onoff_client *cli);
 ...
 * * If a stop is in progress, all clients requesting a thread_restart can
 ...
 * already completed.
 */
sword_t onoff_cancel(struct onoff_service *srv,
	struct onoff_client *cli);
