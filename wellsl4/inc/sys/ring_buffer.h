...
#ifndef SYS_RING_BUFFER_H_
#define SYS_RING_BUFFER_H_
...
#define RING_BUF_DECLARE(name, size8) \
	static byte_t _ring_buffer_data_##name[size8]; \
	struct ring_buf name = { \
		.size = size8, \
 ...
 * @param data Ring buffer data area (u32_t data[size] or byte_t data[size] for
 *	       bytes mode).
 */
static FORCE_INLINE void ring_buf_init(struct ring_buf *buf, u32_t size, void *data)
{
 ...
 */
static FORCE_INLINE u32_t ring_buf_custom_space_get(u32_t size, u32_t head,
					      u32_t tail)
{
	if (tail < head) {
		return head - tail - 1;
 ...
 */
static FORCE_INLINE sword_t ring_buf_is_empty(struct ring_buf *buf)
{
 ...
 * @param buf Address of ring buffer.
 */
static FORCE_INLINE void ring_buf_reset(struct ring_buf *buf)
{
	buf->head = 0;
	buf->tail = 0;
 ...
 * @return Ring buffer free space (in 32-bit words or bytes).
 */
static FORCE_INLINE u32_t ring_buf_space_get(struct ring_buf *buf)
{
	return ring_buf_custom_space_get(buf->size, buf->head, buf->tail);
}
 ...
 */
static FORCE_INLINE u32_t ring_buf_capacity_get(struct ring_buf *buf)
{
 ...
 */
sword_t ring_buf_item_put(struct ring_buf *buf, u16_t type, byte_t value,
		      u32_t *data, byte_t size32);
 ...
 */
sword_t ring_buf_item_get(struct ring_buf *buf, u16_t *type, byte_t *value,
		      u32_t *data, byte_t *size32);
 ...
 */
u32_t ring_buf_put_claim(struct ring_buf *buf, byte_t **data, u32_t size);
 ...
 */
sword_t ring_buf_put_finish(struct ring_buf *buf, u32_t size);
 ...
 */
u32_t ring_buf_put(struct ring_buf *buf, const byte_t *data, u32_t size);
 ...
 */
u32_t ring_buf_get_claim(struct ring_buf *buf, byte_t **data, u32_t size);
 ...
 */
sword_t ring_buf_get_finish(struct ring_buf *buf, u32_t size);
 ...
 */
u32_t ring_buf_get(struct ring_buf *buf, byte_t *data, u32_t size);
