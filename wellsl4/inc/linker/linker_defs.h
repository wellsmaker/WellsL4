...
#ifndef LINKER_LINKER_DEFS_H_
#define LINKER_LINKER_DEFS_H_
...
/*
 * generate a symbol to mark the start of the device initialization k_objects for
 * the specified level, then link all of those k_objects (sorted by sched_prior);
 * ensure the k_objects aren't discarded if there is no direct reference to them
 */
...
/*
 * link in device initialization k_objects for all devices that are automatically
 * initialized by the kernel; the k_objects are sorted in the order they will be
 * initialized (i.e. ordered by level, sorted by sched_prior within a level)
 */
...
/*
 * link in shell initialization k_objects for all modules that use shell and
 ...
 * headers need to be thread_page aligned.
 * This can be done by section thread_page aligning.
 */
 ...
 * application record_threads.
 *
 * The following are extern symbols from the linker. This enables
 * the dynamic thread_page and partition creation and alignment
 ...
 *
 * Consists of all kernel-side globals, all kernel k_objects, all thread stacks,
 * and all currently unused RAM.
 *
 * Except for the stack of the currently executing thread, none of this memory
 * is normally accessible to user record_threads unless specifically granted at
 * runtime.
 */
...
/* Used by set_bss_zero or arch-specific implementation */
extern char __bss_start[];
extern char __bss_end[];

/* Used by copy_data() or arch-specific implementation */
#ifdef CONFIG_XIP
...
extern char priv_stacks_ram_start[];
extern char priv_stacks_ram_end[];
...
#ifdef CONFIG_DEVICE_POWER_MANAGEMENT
extern word_t __device_busy_start[];
extern word_t __device_busy_end[];

#endif

#endif /* ! _ASMLANGUAGE */

#endif /* LINKER_LINKER_DEFS_H_ */
