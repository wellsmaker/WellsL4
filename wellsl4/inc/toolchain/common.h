...
#ifndef TOOLCHAIN_COMMON_H_
#define TOOLCHAIN_COMMON_H_
/**
 * @file
...
/* Use TASENTRY_CPP to tag task entry points defined in C++ files. */

#ifdef __cplusplus
#define TASENTRY_CPP  extern "C"
#endif
...
/* force inlining a function */
     ...
     * even ignoring -fno-FORCE_INLINE. So for code coverage, do not
     * force inlining of these functions to keep their bodies around
     * so their number of executions can be counted.
     *
     * Note that "FORCE_INLINE" is kept here for kobject_hash.c and
     * priv_stacks_hash.c. These are built without compiler flag
     * used for coverage. FORCE_INLINE cannot be empty as compiler
     ...
     * So just keep "FORCE_INLINE" here.
     */
 ...
 * until https://github.com/wellsl4project-rtos/wellsl4/issues/5006 is
...
#define STRUCT_SECTION_ITERABLE(struct_type, name) \
	DECL_ALIGN(struct struct_type) name \
	__in_section(_##struct_type, static, name) __used

/*
 * Itterator for structure instances gathered by STRUCT_SECTION_ITERABLE().
 ...
 * list of struct k_objects to iterate over.
 */
#define STRUCT_SECTION_FOREACH(struct_type, iterator) \
	...
	for (struct struct_type *iterator = \
	     ...
	     ({ assert_info(iterator <= _CONCAT(_##struct_type, _list_end), \
			 "unexpected list end location"); \
		iterator < _CONCAT(_##struct_type, _list_end); }); \
	     iterator++)

#endif /* TOOLCHAIN_COMMON_H_ */
