...
#define DEVICE_AND_API_INIT(dev_name, drv_name, init_fn, data, cfg_info,  \
	...
	static DECL_ALIGN(struct device) _CONCAT(__device_, dev_name) __used \
...
#define DEVICE_AND_API_INIT(dev_name, drv_name, init_fn, data, cfg_info, \
	...
	DEVICE_DEFINE(dev_name, drv_name, init_fn,			 \
		      do_device_pm_ctrl_nop, data, cfg_info, level,	 \
		      prio, api)
#endif
 ...
 * instead.
 *
 * @param dev_name Device name. This must be less than DEVICE_MAX_NAME_LEN
 ...
 *
 * @param prio The initialization sched_prior of the device, relative to
 ...
 * Can be empty function (do_device_pm_ctrl_nop) if not implemented.
 */
#ifndef CONFIG_DEVICE_POWER_MANAGEMENT
...
#define DEVICE_DEFINE(dev_name, drv_name, init_fn, pm_control_fn,	  \
	...
	static struct device_pm _CONCAT(__pm_, dev_name) __used = {  \
		.usage = ATOMIC_INIT(0),                                  \
		.lock = SEM_INITIALIZER(                               \
				_CONCAT(__pm_, dev_name).lock, 1, 1),     \
		.signal = POLL_SIGNAL_INITIALIZER(                      \
				_CONCAT(__pm_, dev_name).signal),         \
		.event = POLL_EVENT_INITIALIZER(POLL_TYPE_SIGNAL,     \
				POLL_MODE_NOTIFY_ONLY,                  \
				&_CONCAT(__pm_, dev_name).signal),        \
	};								  \
	static struct device_config _CONCAT(__config_, dev_name) __used	  \
	__attribute__((__section__(".devconfig.init"))) = {		  \
 ...
 * k_objects before using the DEVICE_GET macro to get the device object.
 *
 ...
 */
#define DEVICE_GET_NAME(name) (_CONCAT(__device_, name))
 ...
 */
#define DEVICE_GET(name) (&DEVICE_GET_NAME(name))
 ...
 */
#define DEVICE_DECLARE(name) static struct device DEVICE_GET_NAME(name)
 ...
 */
#define SYS_NAME(init_fn) _CONCAT(_CONCAT(do_init_, init_fn), __COUNTER__)

/**
 * @def SYS_INIT
 *
 * @brief Run an initialization function at boot at specified sched_prior
 *
...
#define SYS_INIT(init_fn, level, prio) \
	DEVICE_AND_API_INIT(SYS_NAME(init_fn), "", init_fn, NULL, NULL, level,\
	prio, NULL)
 ...
 * @param init_fn Pointer to the boot function to run
 * @param pm_control_fn Pointer to device_pm_control function.
 *                      Can be empty function (do_device_pm_ctrl_nop) if not
 *                      implemented.
...
#define SYS_DEVICE_DEFINE(drv_name, init_fn, pm_control_fn, level, prio) \
	DEVICE_DEFINE(SYS_NAME(init_fn), drv_name, init_fn, pm_control_fn, \
		      NULL, NULL, level, prio, NULL)

struct device;
typedef void (*device_pm_cb)(struct device *dev, s32_t status, void *context, void *arg);
...
struct device_config {
	...
	s32_t (*device_pm_control)(struct device *dev, 
		u32_t ctrl, void *context, device_pm_cb cb, void *arg);
	struct device_pm *pm;
#endif
};
 ...
 *
 * @details Device k_objects are created via the DEVICE_INIT() macro and
 ...
 */
/*
__syscall struct device *device_binding(const char *name) 
{ 
	return NULL;
}
*/
 ...
 */
void do_device_set_busy(struct device *busy_dev);
 ...
 * @param busy_dev Pointer to device structure of the driver instance.
 */
void do_device_clear_busy(struct device *busy_dev);
 ...
 */
s32_t do_device_pm_ctrl_nop(struct device *unused_device,
 ...
 */
static FORCE_INLINE s32_t device_set_power_state(struct device *device,
 ...
 */
static FORCE_INLINE s32_t device_get_power_state(struct device *device,
					 u32_t *device_power_state)
{
	return device->config->device_pm_control(device,
 ...
 */
void do_device_get(struct device **device_list, s32_t *device_count);
 ...
 */
s32_t do_device_check_busy(struct device *chk_dev);
 ...
 * @retval Errno Negative errno code if failure.
 */
s32_t device_pm_get(struct device *dev);
 ...
 * @retval Errno Negative errno code if failure.
 */
s32_t device_pm_get_sync(struct device *dev);
 ...
 * @retval Errno Negative errno code if failure.
 */
s32_t device_pm_put(struct device *dev);
 ...
 * @retval Errno Negative errno code if failure.
 */
s32_t device_pm_put_sync(struct device *dev);
#else
static FORCE_INLINE void device_pm_enable(struct device *dev) { }
static FORCE_INLINE void device_pm_disable(struct device *dev) { }
static FORCE_INLINE s32_t device_pm_get(struct device *dev) { return -ENOTSUP; }
static FORCE_INLINE s32_t device_pm_get_sync(struct device *dev) { return -ENOTSUP; }
static FORCE_INLINE s32_t device_pm_put(struct device *dev) { return -ENOTSUP; }
static FORCE_INLINE s32_t device_pm_put_sync(struct device *dev) { return -ENOTSUP; }
#endif
#endif
