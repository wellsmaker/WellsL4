...
#ifndef ARCH_ARM_INCLUDE_AARCH32_CORTEX_M_TH_
#define ARCH_ARM_INCLUDE_AARCH32_CORTEX_M_TH_
 ...
 * state configuration.
 */
typedef struct tnonsecure_setup_conf {
	u32_t msp_ns;
	u32_t psp_ns;
	u32_t vtor_ns;
	struct {
 ...
 * enabling Secure exception sched_prior boosting.
 *
 * @param secure_boost Flag indicating whether Secure sched_prior boosting
 *                 is desired; select 1 for sched_prior boosting, otherwise 0.
 *
 * Note:
 *
 * This function shall only be called from Secure state.
 ...
 *
 * @param p_sau_conf pointer to a tsau_conf_t structure
 *
 * This function shall only be called from Secure state, otherwise the
 ...
 */
sword_t tsau_region_configure(tsau_conf_t *p_sau_conf);
 ...
 */
typedef void __attribute__((cmse_nonsecure_call)) (*tns_func_ptr_t) (void);
 ...
 */
#define __TNONSECURE_ENTRY_FUNC \
 ...
 */
#define TNONSECURE_FUNC_PTR_DECLARE(fptr) tns_func_ptr_t fptr
 ...
 */
#define TNONSECURE_FUNC_PTR_CREATE(fptr) \
	((tns_func_ptr_t)(cmse_nsfptr_create(fptr)))
 ...
 */
#define TNONSECURE_FUNC_PTR_IS_NS(fptr) \
	cmse_is_nsfptr(fptr)
