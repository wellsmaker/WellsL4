...
#define STRONGLY_ORDERED_SHAREABLE \   
		MPU_RASR_S_Msk
#define DEVICE_SHAREABLE \
		(MPU_RASR_B_Msk | MPU_RASR_S_Msk)
...
#define NORMAL_OUTER_INNER_WRITE_THROUGH_NON_SHAREABLE \
		MPU_RASR_C_Msk
#define NORMAL_OUTER_INNER_WRITE_BACSHAREABLE	\
		(MPU_RASR_C_Msk | MPU_RASR_B_Msk | MPU_RASR_S_Msk)
#define NORMAL_OUTER_INNER_WRITE_BACNON_SHAREABLE \
...
#define NORMAL_OUTER_INNER_NON_CACHEABLE_NON_SHAREABLE \
		(1 << MPU_RASR_TEX_Pos)
#define NORMAL_OUTER_INNER_WRITE_BACWRITE_READ_ALLOCATE_SHAREABLE \
		((1 << MPU_RASR_TEX_Pos) | MPU_RASR_C_Msk | MPU_RASR_B_Msk | MPU_RASR_S_Msk)
#define NORMAL_OUTER_INNER_WRITE_BACWRITE_READ_ALLOCATE_NON_SHAREABLE \
...
#define REGION_SIZE(size) \
	((ARM_MPU_REGION_SIZE_ ## size << MPU_RASR_SIZE_Pos) & MPU_RASR_SIZE_Msk)
...
#define REGION_RAM_ATTR(size) { \
		(NORMAL_OUTER_INNER_WRITE_BACWRITE_READ_ALLOCATE_NON_SHAREABLE | \
...
#define REGION_PPB_ATTR(size) { \
		(STRONGLY_ORDERED_SHAREABLE | size | P_RW_U_NA_Msk) \
	}
#define REGION_IO_ATTR(size) { \
		(DEVICE_NON_SHAREABLE | size | P_RW_U_NA_Msk) \
	}
 ...
 * k_objects. The format of partition_attr_t is an
...
#define MEM_PARTITION_P_RWX_U_RWX	((partition_attr_t) \
	{ _MEM_PARTITION_P_RWX_U_RWX | \
	  NORMAL_OUTER_INNER_WRITE_BACWRITE_READ_ALLOCATE_NON_SHAREABLE})
#define MEM_PARTITION_P_RWX_U_RX	((partition_attr_t) \
 ...
 *             allowance.
 */
#define PARTITION_IS_EXECUTABLE(attr) (!((attr.rasr_attr) & (NOT_EXEC)))

/* Attributes for no-cache enabling (share-ability is selected by default) */

#define MEM_PARTITION_P_NA_U_NA_NOCACHE ((partition_attr_t) \
	{(_MEM_PARTITION_P_NA_U_NA \
	| NORMAL_OUTER_INNER_NON_CACHEABLE_SHAREABLE)})
#define MEM_PARTITION_P_RW_U_RW_NOCACHE ((partition_attr_t) \
	{(_MEM_PARTITION_P_RW_U_RW \
	| NORMAL_OUTER_INNER_NON_CACHEABLE_SHAREABLE)})
#define MEM_PARTITION_P_RW_U_RO_NOCACHE ((partition_attr_t) \
	{(_MEM_PARTITION_P_RW_U_RO \
	| NORMAL_OUTER_INNER_NON_CACHEABLE_SHAREABLE)})
...
#define MEM_PARTITION_P_RWX_U_RWX_NOCACHE ((partition_attr_t) \
	{(_MEM_PARTITION_P_RWX_U_RWX \
	| NORMAL_OUTER_INNER_NON_CACHEABLE_SHAREABLE)})
#define MEM_PARTITION_P_RWX_U_RX_NOCACHE  ((partition_attr_t) \
	{(_MEM_PARTITION_P_RWX_U_RX  \
	| NORMAL_OUTER_INNER_NON_CACHEABLE_SHAREABLE)})
