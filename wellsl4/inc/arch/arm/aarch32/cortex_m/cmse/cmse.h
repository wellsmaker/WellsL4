...
#ifndef ARCH_ARM_INCLUDE_AARCH32_CORTEX_M_CMSE_H_
#define ARCH_ARM_INCLUDE_AARCH32_CORTEX_M_CMSE_H_
 ...
 */
sword_t arm_cmse_mpu_region_get(u32_t addr);

/**
 * @brief Read accessibility of an address
 *
 * Evaluates whether a specified memory location can be read according to the
 * permissions of the _current_thread state MPU and the specified operation mode.
 *
 ...
 *                    for unprivileged access, regardless of whether the _current_thread
 *                    mode is privileged or unprivileged.
 *
 * @return 1 if address is readable, 0 otherwise.
 */
sword_t arm_cmse_addr_read_ok(u32_t addr, sword_t force_npriv);
 ...
 * to the permissions of the _current_thread state MPU and the specified operation
 * mode.
 *
 * This function shall always return zero:
 * - if executed from an unprivileged mode,
 ...
 */
sword_t arm_cmse_addr_readwrite_ok(u32_t addr, sword_t force_npriv);

/**
 * @brief Read accessibility of an address range
 *
 * Evaluates whether a memory address range, specified by its base address
 * and size, can be read according to the permissions of the _current_thread state MPU
 ...
 */
sword_t arm_cmse_addr_range_read_ok(u32_t addr, u32_t size, sword_t force_npriv);

/**
 * @brief Read and Write accessibility of an address range
 *
 * Evaluates whether a memory address range, specified by its base address
 * and size, can be read/written according to the permissions of the _current_thread
 ...
 */
sword_t arm_cmse_addr_range_readwrite_ok(u32_t addr, u32_t size, sword_t force_npriv);
 ...
 * permissions of the _current_thread state MPU.
 *
 ...
 *
 * Evaluates whether a given object can be read according to the
 * permissions of the _current_thread state MPU (unprivileged read).
 *
 * The macro shall always evaluate to zero if called from an unprivileged mode.
 *
 * @param p_obj Pointer to the given object
 ...
 * according to the permissions of the _current_thread state MPU.
 *
 * The macro shall always evaluate to zero if called from an unprivileged mode.
 *
 * @param p_obj Pointer to the given object
 ...
 * to the permissions of the _current_thread state MPU (unprivileged read/write).
 *
 * The macro shall always evaluate to zero if called from an unprivileged mode.
 *
 * @param p_obj Pointer to the given object
 ...
 * @return a valid MPU region number or -EINVAL
  */
sword_t arm_cmse_mpu_nonsecure_region_get(u32_t addr);
 ...
 * @return a valid SAU region number or -EINVAL
  */
sword_t arm_cmse_sau_region_get(u32_t addr);
 ...
 * @return a valid IDAU region number or -EINVAL
  */
sword_t arm_cmse_idau_region_get(u32_t addr);
 ...
 */
sword_t arm_cmse_addr_is_secure(u32_t addr);
 ...
 */
sword_t arm_cmse_addr_nonsecure_read_ok(u32_t addr, sword_t force_npriv);
 ...
 */
sword_t arm_cmse_addr_nonsecure_readwrite_ok(u32_t addr, sword_t force_npriv);
 ...
 * @return 1 if address range is readable, 0 otherwise.
 */
sword_t arm_cmse_addr_range_nonsecure_read_ok(u32_t addr, u32_t size,
	sword_t force_npriv);
 ...
 * @return 1 if address range is readable, 0 otherwise.
 */
sword_t arm_cmse_addr_range_nonsecure_readwrite_ok(u32_t addr, u32_t size,
	sword_t force_npriv);
...
#endif /* ARCH_ARM_INCLUDE_AARCH32_CORTEX_M_CMSE_H_ */
